...
export default function FinancialDashboardPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const finalizadas = obrasData.filter(obra => {
        if (obra.status !== 'finalizada' || !obra.data_fim) return false;
        const dataFim = new Date(obra.data_fim);
        return dataFim >= new Date(startDate) && dataFim <= new Date(endDate);
      });
      setObrasFinalizadas(finalizadas.length);

      // Load financial data for all projects
      const allReceitas: Receita[] = [];
      const allCustos: Custo[] = [];
      const allLancamentos: LancamentoMaoObra[] = [];

      for (const obra of obrasData) {
        const [receitasData, custosData, lancamentosData] = await Promise.all([
          receitasApi.getByObraId(obra.id),
          custosApi.getByObraId(obra.id),
          lancamentosMaoObraApi.getByObraId(obra.id),
        ]);

        allReceitas.push(...receitasData);
        allCustos.push(...custosData);
        allLancamentos.push(...lancamentosData);
      }
      ...
      const filteredCustos = allCustos.filter(c => {
        const data = new Date(c.data);
        return data >= new Date(startDate) && data <= new Date(endDate);
      });

      const filteredLancamentos = allLancamentos.filter(l => {
        const data = new Date(l.data);
        return data >= new Date(startDate) && data <= new Date(endDate);
      });

      setReceitas(filteredReceitas);
      setCustos(filteredCustos);
